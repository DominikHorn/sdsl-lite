...
/*! \file cst_iterators.hpp
    \brief cst_iterators.hpp contains iterator classes for traversing
   (compressed) suffix arrays. \author Simon Gog
*/
...
/*! The cst_dfs_const_forward_iterator iterates through the nodes of a
   (compressed) suffix tree in depth first search (dfs) order. Note, that
          - each inner node is visited twice, and
          - each leaf node is visited only once.

        If the current node is a inner node, the method visit() returns 1 for
   the first visit and 2 for the second one.
...
class cst_dfs_const_forward_iterator {
 public:
  ...
  using difference_type = typename Cst::node_type;
 ...
 private:
  ...
  inline node_type first_child() {
    if (m_stack_cache != nullptr and
        m_stack_size < cache_size)  // push node to the stack
 ...
 public:
  //! Constructor
  cst_dfs_const_forward_iterator(const Cst* cst, const value_type node,
                                 bool visited = false, bool valid = true)
    ...
    if (m_cst == nullptr) {
      ...
      m_stack_size = 0;
      //			std::cerr<<"#creating stack "<<m_cst->lb(m_v)<<"
      //"<<m_cst->rb(m_v)<<std::endl;
    }
  }

  //! Copy Constructor
  //	cst_dfs_const_forward_iterator(const cst_dfs_const_forward_iterator
  //&it):m_cst(it.cst),m_v(it.m_v),m_valid(m_valid),
  // m_stack_cache(nullptr),m_stack_size(0){
  //	}

  ~cst_dfs_const_forward_iterator() {
    if (m_stack_cache != nullptr) {
      //			std::cerr<<"#deleting stack "<<m_cst->lb(m_v)<<"
      //"<<m_cst->rb(m_v)<<std::endl;
  ...
  iterator& operator++() {
    ...
    if (!m_visited) {  // go down, if possible
      if (m_cst->is_leaf(m_v)) {
        ...
        if (w ==
            m_cst->root()) {  // if there exists no right sibling of the leaf v
                              //					w =
                              // m_cst->parent(m_v);
          w = parent();
  ...
  bool operator==(const iterator& it) const {
    return (it.m_visited == m_visited)  // visited status is equal
           and (it.m_valid ==
                m_valid)        // valid status is equal => for end() iterator
...
class cst_bottom_up_const_forward_iterator {
 ...
 public:
  ...
  //! Constructor
  cst_bottom_up_const_forward_iterator(const Cst* cst, const value_type node,
                                       bool valid = true)
      : m_valid(valid) {
    m_cst = cst;
    m_v = node;
  ...
  iterator& operator++() {
    ...
    if (w == m_cst->root()) {    // if no next right sibling exist
      ...
      m_v =
          m_cst->leftmost_leaf(w);  // go to leaftmost leaf in the subtree of w
    }
    return *this;
  }
...
class cst_bfs_iterator {
 ...
 public:
   ...
   * \param end   If valid=true and end=true, we get the end() iterator
   * otherwise ``end'' has no effect.
   */
  cst_bfs_iterator(const Cst* cst, const value_type node, bool valid = true,
                   bool end_it = false) {
    m_cst = cst;
    m_valid = valid;
  ...
  bool operator==(const iterator& it) const {
    if (m_queue.size() !=
        it.m_queue.size()) {  // if the queue size is different
      return false;           // the state of the to iterator are different
    }
    if (m_queue.empty()) {  // if the queue is empty, we have to check if they
                            // are valid and
      return it.m_valid == m_valid and
             it.m_cst == m_cst;  // belong to the same cst
    }
    return (it.m_valid ==
            m_valid)  // valid status is equal => for end() iterator
           and (it.m_cst == m_cst)  // iterator belongs to the same cst
           ...
           and (it.m_queue.back() ==
                m_queue.back());  //  back element are the same.
  }
