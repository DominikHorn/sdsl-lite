...
class cst_node_child_proxy_iterator {
 public:
  ...
  using difference_type = typename t_cst::node_type;
...
class cst_node_child_proxy {
 ...
 public:  // constructors
  ...
  explicit cst_node_child_proxy(const t_cst* cst, node_type v)
      : m_parent(v), m_cst(cst){};
  cst_node_child_proxy(const cst_node_child_proxy& p)
      : m_parent(p.m_parent), m_cst(p.m_cst){};
...
//! Calculate the balanced parentheses of the Super-Cartesian tree, described in
//! Ohlebusch and Gog (SPIRE 2009).
/*! \param vec Random access container for which the Super-Cartesian tree
 * representation should be calculated. The value_type of vec should be an
 * unsigned integer type. \param bp Reference to the balanced parentheses
 * sequence which represents the Super-Cartesian tree. \param minimum Specifies
 * if the higher levels contains minima or maxima. Default is maxima. \par Time
 * complexity \f$ \Order{2n} \f$, where \f$ n=\f$vec.size() \par Space
 * complexity \f$ \Order{n \cdot \log n } \f$ bits.
 */
template <class t_rac>
void construct_supercartesian_tree_bp(const t_rac& vec, bit_vector& bp,
                                      const bool minimum = true) {
  typedef typename t_rac::size_type size_type;
  bp.resize(
      2 *
      vec.size());  // resize bit vector for balanaced parantheses to 2 n bits
  ...
  for (size_type i = 0; i < vec.size(); ++i) {
    ...
    if (minimum) {
      while (vec_stack.size() > 0 and l < vec_stack.top()) {
        vec_stack.pop();
        ++k; /*bp[k++] = 0; bp is already initialized to zero*/  // writing a
                                                                 // closing
                                                                 // parenthesis
      }
...
/*! \param vec Random access container for which the Super-Cartesian tree
 * representation should be calculated. The value_type of vec should be an
 * unsigned integer type. \param minimum Specifies if the higher levels contains
 * minima or maxima. Default is maxima. \return The balanced parentheses
 * sequence representing the Super-Cartesian tree. \par Time complexity \f$
 * \Order{2n} \f$, where \f$ n=\f$vec.size() \par Space complexity
 *       \f$\Order{n}\f$ bits
 */
template <class t_rac>
bit_vector construct_supercartesian_tree_bp_succinct(
    const t_rac& vec, const bool minimum = true) {
  typedef typename t_rac::size_type size_type;
    ...
    if (minimum) {
      ...
      for (size_type i = 1; i < vec.size(); ++i) {
        if (vec[i] < vec[i - 1]) {
          ...
          while (vec_stack.size() > 0 and vec[i] < vec[vec_stack.top()]) {
            vec_stack.pop();
            ++k;  // writing a closing parenthesis, bp is already initialized to
                  // zero
          }
        } else {
      ...
      for (size_type i = 0; i < vec.size(); ++i) {
        while (vec_stack.size() > 0 and vec[i] > vec[vec_stack.top()]) {
          vec_stack.pop();
          ++k;
          /*bp[k++] = 0; bp is already initialized to zero*/  // writing a
                                                              // closing
                                                              // parenthesis
        }
        vec_stack.push(i);
...
//! Calculate the balanced parentheses of the Super-Cartesian tree, described in
//! Ohlebusch and Gog (SPIRE 2009).
/*! \param lcp_buf int_vector_buffer of the LCP Array for which the
 * Super-Cartesian tree representation should be calculated. The value_type of
 * vec should be an unsigned integer type. \param minimum Specifies if the
 * higher levels contains minima or maxima. Default is maxima. \return The
 * balanced parentheses sequence representing the Super-Cartesian tree. \par
 * Time complexity \f$ \Order{2n} \f$, where \f$ n=\f$vec.size() \par Space
 * complexity \f$\Order{2n}\f$ bits, by the multi_stack_support \pre The largest
 * value in lcp_buf has to be smaller than lcp_buf.size().
 */
template <uint8_t t_width>
bit_vector construct_supercartesian_tree_bp_succinct(
    int_vector_buffer<t_width>& lcp_buf, const bool minimum = true) {
...
//! Calculate the balanced parentheses of the Super-Cartesian tree, described in
//! Ohlebusch and Gog (SPIRE 2009) and the first_child bit_vector
/*! \param lcp_buf int_vector_buffer for the lcp array for which the
 * Super-Cartesian tree representation should be calculated. The value_type of
 * vec should be an unsigned integer type. \param bp Reference to the balanced
 * parentheses sequence which represents the Super-Cartesian tree. \param bp_fc
 * Reference to the first child bit_vector of bp. \param minimum Specifies if
 * the higher levels contains minima or maxima. Default is maxima. \par Time
 * complexity \f$ \Order{2n} \f$, where \f$ n=\f$vec.size() \par Space
 * complexity \f$\Order{2n}\f$ bits, by the multi_stack_support
 */
template <uint8_t t_width>
bit_vector::size_type construct_supercartesian_tree_bp_succinct_and_first_child(
    int_vector_buffer<t_width>& lcp_buf, bit_vector& bp, bit_vector& bp_fc,
    const bool minimum = true) {
  typedef bit_vector::size_type size_type;
  ...
  while (!vec_stack.empty()) {
    ...
    // writing a closing parenthesis in bp, not necessary as bp is initialized
    // with zeros
    ++k;
    ++k_fc;
  }
...
typename t_csa::size_type get_char_pos(typename t_csa::size_type idx,
...
struct has_id {
  ...
  static constexpr auto check(T*) -> typename std::is_same<
      decltype(std::declval<T>().id(std::declval<typename T::node_type&>())),
