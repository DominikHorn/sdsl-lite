...
struct int_vector_trait {
  ...
  static const_iterator end(const int_vector_type* v, const uint64_t*,
...
struct int_vector_trait<64> {
  ...
  static iterator end(int_vector_type*, uint64_t* begin,
  ...
  static const_iterator end(const int_vector_type*, const uint64_t* begin,
                            int_vector_size_type size) {
    return begin + size;
  }
...
class int_vector {
 ...
 public:
  ...
  //! Get the integer value of the binary string of length len starting at
  //! position idx in the int_vector.
  /*! \param idx Starting index of the binary representation of the integer.
      \param len Length of the binary representation of the integer. Default
     value is 64. \returns The integer value of the binary string of length len
     starting at position idx. \sa setInt, getBit, setBit
  */
  value_type get_int(size_type idx, const uint8_t len = 64) const;

  //! Set the bits from position idx to idx+len-1 to the binary representation
  //! of integer x.
  /*! The bit at position idx represents the least significant bit(lsb), and the
     bit at position idx+len-1 the most significant bit (msb) of x. \param idx
     Starting index of the binary representation of x. \param x   The integer to
     store in the int_vector. \param len The length used to store x in the
     int_vector. Default value is 64. \sa getInt, getBit, setBit
  */
  ...
  */
  ...
  //! Sets the width of the integers which are accessed via the [] operator, if
  //! t_width equals 0.
  ...
  */
  ...
  size_type serialize(std::ostream& out, structure_tree_node* v = nullptr,
                      std::string name = "",
                      bool write_fixed_as_variable = false) const;
   ...
   * )
   *    - or w[i]==v[i] for all i < min(w.size(), v.size()) and
   * w.size()<v.size(). \sa operator>
   */
  bool operator<(const int_vector& v) const;
  ...
  //! Const iterator that points to the element after the last element of
  //! int_vector.
  ...
  static void read_header(int_vector_size_type& size, int_width_type& int_width,
                          std::istream& in) {
    read_member(size, in);
    if (0 == t_width) {
  ...
  static uint64_t write_header(uint64_t size, uint8_t int_width,
                               std::ostream& out) {
...
//! A proxy class that acts as a reference to an integer of length \p len bits
//! in a int_vector.
...
// special thanks to Timo Beller, who pointed out that the specialization is
// missing Same implementation as in stl_bvector.h.
template <>
...
class int_vector_iterator : public int_vector_iterator_base<t_int_vector> {
 ...
 private:
  using int_vector_iterator_base<t_int_vector>::m_offset;  // make m_offset easy
                                                           // usable
  using int_vector_iterator_base<t_int_vector>::m_len;     // make m_len easy
                                                           // usable
...
// template<class t_int_vector>
// void swap(const int_vector_iterator<t_int_vector> &x, const
// int_vector_iterator<t_int_vector> &y){
//   x->swap(*y);
// }

template <class t_int_vector>
inline int_vector_iterator<t_int_vector> operator+(
    typename int_vector_iterator<t_int_vector>::difference_type n,
...
template <class t_int_vector>
class int_vector_const_iterator
    : public int_vector_iterator_base<t_int_vector> {
 public:
  ...
  template <class X>
  friend typename int_vector_const_iterator<X>::difference_type operator-(
      const int_vector_const_iterator<X>& x,
 ...
 public:
  ...
  const_reference operator*() const {
    ...
    return ((*m_word) >> m_offset) |
           ((*(m_word + 1) & bits::lo_set[(m_offset + m_len) & 0x3F])
            << (64 - m_offset));
  }
...
inline typename std::enable_if<
    std::is_same<typename t_bv::index_category, bv_tag>::value,
    std::ostream&>::type
...
template <uint8_t t_width>
inline int_vector<t_width>::int_vector(size_type size, value_type default_value,
                                       uint8_t intWidth)
...
template <uint8_t t_width>
auto int_vector<t_width>::get_int(size_type idx, const uint8_t len) const
    -> value_type {
#ifdef SDSL_DEBUG
  if (idx + len > m_size) {
    throw std::out_of_range(
        "OUT_OF_RANGE_ERROR: int_vector::get_int(size_type, uint8_t); idx+len "
        "> size()!");
  }
  if (len > 64) {
    throw std::out_of_range(
...
template <uint8_t t_width>
inline void int_vector<t_width>::set_int(size_type idx, value_type x,
                                         const uint8_t len) {
#ifdef SDSL_DEBUG
  if (idx + len > m_size) {
    throw std::out_of_range(
        "OUT_OF_RANGE_ERROR: int_vector::set_int(size_type, uint8_t); idx+len "
        "> size()!");
  }
  if (len > 64) {
    throw std::out_of_range(
...
template <uint8_t t_width>
inline auto int_vector<t_width>::operator[](const size_type& idx) const
    -> const_reference {
  assert(idx < this->size());
...
template <>
inline auto int_vector<0>::operator[](const size_type& idx) const
    -> const_reference {
  assert(idx < this->size());
...
template <>
inline auto int_vector<64>::operator[](const size_type& idx) const
    -> const_reference {
  assert(idx < this->size());
  return *(this->m_data + idx);
}
...
template <>
inline auto int_vector<32>::operator[](const size_type& idx) const
    -> const_reference {
  assert(idx < this->size());
  return *(((uint32_t*)this->m_data) + idx);
}
...
template <>
inline auto int_vector<16>::operator[](const size_type& idx) const
    -> const_reference {
  assert(idx < this->size());
  return *(((uint16_t*)this->m_data) + idx);
}
...
template <>
inline auto int_vector<8>::operator[](const size_type& idx) const
    -> const_reference {
  assert(idx < this->size());
  return *(((uint8_t*)this->m_data) + idx);
}
...
template <>
inline auto int_vector<1>::operator[](const size_type& idx) const
    -> const_reference {
  assert(idx < this->size());
...
bool int_vector<t_width>::operator<(const int_vector& v) const {
  ...
  for (auto it = begin(), end = begin() + min_size, it_v = v.begin(); it != end;
       ++it, ++it_v) {
    if (*it == *it_v)
      continue;
    else
...
int_vector<t_width>& int_vector<t_width>::operator&=(const int_vector& v) {
  ...
  for (uint64_t i = 0; i < (v.capacity() >> 6); ++i) m_data[i] &= v.m_data[i];
  return *this;
}
...
template <uint8_t t_width>
typename int_vector<t_width>::size_type int_vector<t_width>::write_data(
...
template <uint8_t t_width>
typename int_vector<t_width>::size_type int_vector<t_width>::serialize(
    std::ostream& out, structure_tree_node* v, std::string name,
