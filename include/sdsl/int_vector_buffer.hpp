...
class int_vector_buffer {
 ...
 private:
  ...
  // length of int_vector header in bytes: 0 for plain, 8 for
  // int_vector<t_width> (0 < t_width), 9 for int_vector<0>
  uint64_t m_offset = 0;
  uint64_t m_buffersize =
      8;  // in elements! m_buffersize*width() must be a multiple of 8!
 ...
 public:
   ...
   *  \param mode       Openmode:
   *                    std::ios::in opens an existing file (that must exist
   * already), std::ios::out creates a new file (that may exist already). \param
   * buffersize Buffersize in bytes. This has to be a multiple of 8, if not the
   * next multiple of 8 will be taken \param int_width  The width of each
   * integer. \param is_plain   If false (default) the file will be interpreted
   * as int_vector. If true the file will be interpreted as plain array with
   * t_width bits per integer. In second case (is_plain==true), t_width must be
   * 8, 16, 32 or 64.
   */
  ...
  void buffersize(uint64_t buffersize) {
    ...
    if (0 == (buffersize * 8) % width()) {
      m_buffersize =
          buffersize * 8 / width();  // m_buffersize might not be multiple of 8,
                                     // but m_buffersize*width() is.
    } else {
      uint64_t element_buffersize =
          (buffersize * 8) / width() +
          1;  // one more element than fits into given buffersize in byte
      m_buffersize = element_buffersize + 7 -
                     (element_buffersize + 7) % 8;  // take next multiple of 8
    }
  ...
  bool is_open() { return m_ifile.is_open() and m_ofile.is_open(); }
  ...
  /*! It is not possible to read from / write into the int_vector_buffer after
   * calling this method \param remove_file If true, the underlying file will be
   * removed on closing.
   */
  void close(bool remove_file = false) {
    if (is_open()) {
      if (!remove_file) {
        write_block();
        if (0 < m_offset) {  // in case of int_vector, write header and trailing
                             // zeros
  ...
  void swap(int_vector_buffer<t_width>& ivb) {
    if (this != &ivb) {
      ...
      ivb.m_ofile.open(ivb.m_filename,
